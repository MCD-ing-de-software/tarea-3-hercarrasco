...
class TestStatisticsUtils(unittest.TestCase):
    ...
    def test_moving_average_raises_for_invalid_window(self):
        ...
        # Windows > longitud del array
        with self.assertRaises(ValueError):
            utils.moving_average(arr, window=6)
    ...
    def test_zscore_has_mean_zero_and_unit_std(self):
        ...
        # Media 0
        self.assertAlmostEqual(np.mean(z), 0.0, places=8)

        # Std casi 1
        self.assertAlmostEqual(np.std(z), 1.0, places= 8)
    ...
    def test_min_max_scale_maps_to_zero_one_range(self):
        ...
        results = utils.min_max_scale(arr)
        expected = np.array([0.0, 0.5, 1.0])

        self.assertAlmostEqual(results.min(), 0.0)
        self.assertAlmostEqual(results.max(), 1.0)
